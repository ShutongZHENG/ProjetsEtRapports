...
int main(int argc, char *argv[])
{
  ...
  int bufsize, overhead, typesize;  // 缓冲区的大小和其他参数
  double *buffer;                   // 缓冲区
  ...
  MPI_Type_size(MPI_DOUBLE, &typesize);  // 获取MPI_DOUBLE类型的字节大小
  overhead = (int)(1 + (MPI_BSEND_OVERHEAD * 1.) / typesize);  // 计算缓冲区的开销
  buffer = (double *)malloc((nb_tests * (nb_values_max + overhead)) * sizeof(double));  // 分配缓冲区内存
  bufsize = typesize * nb_tests * (nb_values_max + overhead);  // 计算缓冲区的大小
  MPI_Buffer_attach(buffer, bufsize);  // 将缓冲区与MPI环境关联
  ...
  for (iter = 0; iter < nb_tests; iter++)  // 迭代进行每个测试
  {
    if (rank == 0)  // 如果当前进程是进程0
    {
      ...
      MPI_Bsend(values, nb_values[iter], MPI_DOUBLE, 1, tag, MPI_COMM_WORLD);  // 使用缓冲发送消息给进程1
      MPI_Recv(values, nb_values[iter], MPI_DOUBLE, 1, tag, MPI_COMM_WORLD, &status);  // 接收进程1发送的消息
    ...
    {
      MPI_Bsend(values, nb_values[iter], MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);  // 使用缓冲发送消息给进程0
      MPI_Recv(values, nb_values[iter], MPI_DOUBLE, 0, tag,
               MPI_COMM_WORLD, &status);  // 接收进程0发送的消息
    }
  }

  MPI_Buffer_detach(buffer, &bufsize);  // 解除缓冲区与MPI环境的关联
  free(buffer);  // 释放缓冲区内存
  free(values);  // 释放值数组内存
